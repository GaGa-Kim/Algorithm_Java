...
/*
 * 문제 분석하기
 * : 모든 컴퓨터를 연결할 수 있는 최소 신장 트리를 찾도록 함
 */
...
public class Main {
    ...
    static int N, M;
    static int[] parent; // parent(각 컴퓨터의 대표 노드 저장 배열)
    static PriorityQueue<Edge> pq; // pq(간선을 담을 최소값 우선순위 큐)
    static int answer = 0; // answer(모든 컴퓨터를 연결하는데 필요한 최소 비용)
    ...
    static void init() throws IOException {
        ...
        parent = new int[N + 1]; // 정점에 따른 대표 노드 초기화하기
        for (int i = 1; i <= N; i++) {
            parent[i] = i;
        }
